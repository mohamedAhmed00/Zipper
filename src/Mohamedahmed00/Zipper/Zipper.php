...
namespace Mohamedahmed00\Zipper;

use Mohamedahmed00\Zipper\Repositories\RepositoryInterface;
use Exception;
...
class Zipper
{
    ...
    public function make($pathToFile, $type = 'zip')
    {
        ...
        $objectOrName = $type;
        if (is_string($type)) {
            $objectOrName = 'Mohamedahmed00\Zipper\Repositories\\' . ucwords($type) . 'Repository';
        }

        if (!is_subclass_of($objectOrName, 'Mohamedahmed00\Zipper\Repositories\RepositoryInterface')) {
